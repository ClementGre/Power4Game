...
class GameCanvas(tk.Canvas):
    ...
    def on_mouse_click(self, event):
        ...
        self.game.play(x, True)
        for x in range(7):
            for y in range(6):
                self.set_token(x, y, self.game.grid[y][x])
        pass

    def create_repeated_image(self, image, width, height):
    ...
    def create_widgets(self):
        ...
        for x in range(7):
            for y in range(6):
                self.set_token(x, y, None)

        # Draw the pieces: 8ux7u red or yellow circles
    ...
    def set_token(self, x, y, player):
        ...
        self.create_oval(self.PANEL_MARGIN + x * self.UNIT - self.HOLE_MARGIN,
                         ...
                         fill=color, outline="#506CE3", width=3, tags=f"{x}_{y}")
    ...
    def setup_background(self, width, height):
        ...
        repeated_image = self.create_repeated_image(self.bg_image, width, height)
